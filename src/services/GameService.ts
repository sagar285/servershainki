import { GameState, Question, SubmissionResult, SocketUser } from "../types";
...
export class GameService {
  ...
  private submissionLock: boolean = false;
  private currentDifficulty: "easy" | "medium" | "hard" = "medium";
  private questionChangeCallback:
    | ((newQuestion: Question, gameState: GameState) => void)
    | null = null;
  private winnerAnnouncementCallback:
    | ((winner: any, correctAnswer: number) => void)
    | null = null;
  ...
  setQuestionChangeCallback(
    callback: (newQuestion: Question, gameState: GameState) => void
  ): void {
    this.questionChangeCallback = callback;
  }

  setWinnerAnnouncementCallback(
    callback: (winner: any, correctAnswer: number) => void
  ): void {
  ...
  async submitAnswer(
    ...
    if (
      ...
      console.log(
        `❌ Game not active or no question. Active: ${
          this.gameState.isActive
    ...
    const timeTaken =
      submissionTime.getTime() - this.gameState.startTime.getTime();
    ...
    console.log(
      `🔍 Answer check: Expected ${expectedAnswer}, Got ${answer}, Correct: ${isCorrect}`
    );
    ...
    if (this.submissionLock || this.gameState.winner) {
      console.log(
        `⏳ Submission rejected: Lock ${
          this.submissionLock
        }, Winner exists: ${!!this.gameState.winner}`
      );
      return {
        isCorrect: true,
        isWinner: false,
    ...
    try {
      ...
      if (this.winnerAnnouncementCallback) {
        ...
        this.winnerAnnouncementCallback(
          { userId, username, timeTaken },
  ...
  private async updateUserScore(
    ...
    try {
      ...
      console.log(
        `Updated score for ${username}: +${totalScore} points (${timeTaken}ms)`
      );
    } catch (error) {
  ...
  startNewQuestion(): void {
    ...
    const question = QuestionGenerator.generateQuestion(this.currentDifficulty);
    ...
    console.log(
      `📊 New ${this.currentDifficulty} question: ${question.problem} (Answer: ${question.answer})`
    );
    console.log(
      `👥 Broadcasting to ${this.connectedUsers.size} connected users`
    );
  ...
  private adjustDifficulty(): void {
    ...
    if (previousDifficulty !== this.currentDifficulty) {
      console.log(
        `🎚️ Difficulty adjusted to ${this.currentDifficulty} (${participantCount} participants)`
      );
    }
  }
  ...
  addUser(socketId: string, userId: string, username: string): void {
    ...
    if (this.connectedUsers.size === 1 || !this.gameState.currentQuestion) {
      console.log(
        `🚀 Starting new question for ${this.connectedUsers.size} users`
      );
      setTimeout(() => {
        this.startNewQuestion();
      }, 500);
