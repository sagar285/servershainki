...
// Submit answer
router.post("/submit", rateLimiter_1.answerSubmissionLimiter, async (req, res) => {
    try {
        ...
        if (typeof answer !== "number" ||
            ...
            res.status(400).json({
                error: "Invalid submission. Answer must be a number and username is required.",
            });
            return;
        }
        ...
        const result = await gameService.submitAnswer(userId, username.trim(), answer);
        res.json({
            ...result,
...
router.post("/user", rateLimiter_1.userCreationLimiter, async (req, res) => {
    try {
